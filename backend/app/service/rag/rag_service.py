...
class RagService: #친절한 주석 < -RAG서비스
    ...
    def _generate_with_local_llm(self, query: str, context_str: str) -> str:
        ...
        messages = [
            {
                "role": "system", 
                "content": "당신은 Hot's POD의 친절한 AI 어시스턴트입니다. 현재는 2025년입니다. 소모임 정보를 바탕으로 자연스럽고 친근하게 답변하세요. [1번], [2번] 같은 번호는 사용하지 말고 [장소] 같이 제공된 정보에 포함된 [] 안에 있는 태그정보 또한 답변에 직접 노출시키지 마세요, **, # 같은 마크다운 문법을 사용하지말고 일반 텍스트의 형태로만 답변을 제공하세요."
            },
            {
                "role": "user", 
